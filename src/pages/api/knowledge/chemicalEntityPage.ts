...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  console.log('page', req.body['chemicalsName'], req.body['chemicalTypes']);

  const res_ = await fetch(
    `${process.env.NEXT_PUBLIC_ANALYTICS_Ms_Gateway}/cx-scws/chemicalsEntity/page?${page}`,
    {
      method: 'post',
      headers: {
      ...
      body: JSON.stringify({
        chemicalTypes: req.body['chemicalTypes'],
        chemicalsName: req.body['chemicalsName'],
      }),
    }
  );
  ...
  console.log('结果-chemicalsEntity', status, json);
