...
import {
  ...
  alarmTypeModel,
  currentAlarmModel,
  currentBoradcastModel,
  currentDpAlarmModel,
  currentDpIncidentModel,
  currentEmgcModel,
  currentEscapeModel,
  currentIncidentIdModel,
  currentPartitionIdsModel,
  currentPlanModel,
  currentSurroundModel,
  departModal,
  IAlarmDetail,
  ISuppData,
...
import { largeScreenModal } from '@/models/baseWebSocket';
...
const Websocket = (_: object, refs: Ref<Refs>) => {
  ...
  const setCurrentIncidentId = useSetRecoilState(currentIncidentIdModel);
  ...
  const setCurrentEmgc = useSetRecoilState(currentEmgcModel);
  const setCurrentPlan = useSetRecoilState(currentPlanModel);
  const setCurrentEscape = useSetRecoilState(currentEscapeModel);
  const setCurrentSurround = useSetRecoilState(currentSurroundModel);
  const setCurrentBoradcast = useSetRecoilState(currentBoradcastModel);
  const setCurrentPartitionIds = useSetRecoilState(currentPartitionIdsModel);
  ...
  const departmentData = useRecoilValue(departModal);

  // 是否静音
  const [audioState, setAudioState] = useLocalStorageState('state', { defaultValue: 'play' });

  const alarmType = useRecoilValue(alarmTypeModel);
  const alarmTypeDp = useRecoilValue(alarmTypeDpModel);
  ...
  // const renderEarthquakeToast = useMemoizedFn((alarm: earthquakeType) => {
  //   const delay = Date.now() - alarm.updateAt;
  //   const countDown = ''
  ...
  const handleOnMsg = useMemoizedFn(({ data }) => {
    try {
      ...
      // console.info('============msgBody==============', msgBody);
      if (msg.code && msg.code === 500) {
        // console.info('code 500', msg);
        socket.current?.close();
      ...
      // console.info('============alarmType2==============', alarmType);
        ...
        if (isAuth > -1) {
          switch (type) {
            case 'ALARM':
              ...
              if (
                !newMsg.deptId ||
                departmentData.includes(newMsg.deptId) ||
            ...
            case 'CLOSE_ALARM':
              if (
                window &&
                (window.location.pathname.includes('montior/operation') ||
                  window.location.pathname.includes('sms/operation'))
              ) {
      ...
      if (topic === 'slave_large_status') {
        const newMsg = JSON.parse(msgBody);
        const newLargeScreen = { ...largeScreen };
        switch (newMsg.operationAction) {
          case 'ENTRY_EMGC': // 进入应急态
            setCurrentEmgc(true);
            setCurrentIncidentId(newMsg.incidentId);
            break;
          case 'EXIT_EMGC': // 退出应急态
            setCurrentEmgc(false);
            break;
          case 'UPDATE_PLAN': // 应急预案消息推送
            setCurrentPlan(new Date().getTime());
            break;
          case 'ESCAPE': // 逃生路线打开/关闭消息推送
            setCurrentEscape(!newMsg.removeKeep);
            break;
          case 'SURROUND': // 查周边
            setCurrentSurround(!newMsg.removeKeep);
            break;
          case 'BROADCAST': // 广播
            setCurrentBoradcast(!newMsg.removeKeep);
            setCurrentPartitionIds(newMsg.param.partitionIds);
            break;
          case 'MAP':
            newLargeScreen[newMsg.operationAction] = { ...newMsg, update: v4() };
  ...
  const initWebSocket = useMemoizedFn((clientType: IClientType = 'web-show') => {
    ...
    const url = dev
      ? `${protocol}://${process.env.NEXT_PUBLIC_ANALYTICS_Pt_message}/websocket?userId=${userId}&clientType=${clientType}`
      : `${protocol}://${process.env.NEXT_PUBLIC_ANALYTICS_Pt_message}/websocket?userId=${userId}&clientType=${clientType}`;

    //socket.current = new WebSocket('ws://127.0.0.1:8888');
  ...
  const addAlarm = useMemoizedFn(async (alarm: IAlarmDetail) => {
    //需要接受报警推送的页面
    const montiorRouter = ['montior/operation', 'sms/operation', 'largeScreen'];
    //需要接受报警推送的页面（不包括大屏）
    const montiorRouterNotLG = ['montior/operation', 'sms/operation'];
    //是否处于检测预警页面
