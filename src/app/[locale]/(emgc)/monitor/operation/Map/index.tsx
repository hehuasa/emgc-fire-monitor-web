...
import { center, featureCollection, lineString, point, polygon } from '@turf/turf';
...
const Map = ({ getMapObj }: IProps) => {
  ...
  // 区域点击事件
  const handleAreaClick = useMemoizedFn((e: IL7LayerEventTarget) => {
  ...
  return (
    <>
      ...
      {showPopup && mapRef.current && currentAreaClusterData && (
        <Popup lngLat={popuplnglat} onClose={handlePopupClose}>
          <div
            onMouseEnter={() => {
