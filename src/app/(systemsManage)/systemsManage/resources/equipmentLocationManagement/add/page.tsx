...
import type { IDevice } from '../deviceTable';
...
const Edit = ({
  ...
  const selectDevice = useMemoizedFn((item: IDevice) => {
    deviceOnClose();
    setValue('resourceName', item.name, { shouldValidate: isSubmitted ? true : false });
    setValue('iotDeviceId', item.deviceId, { shouldValidate: isSubmitted ? true : false });
    setValue('iotSubDeviceId', item.subDeviceCode, { shouldValidate: isSubmitted ? true : false });
  });
    ...
    <>
      <Modal isOpen={isOpen} onClose={onClose} size="lg" onCloseComplete={closeModal}>
        ...
        <ModalContent w="1000px" maxW="unset" pb="20px">
          <ModalHeader>设备（资源）管理</ModalHeader>
                  ...
                  >
                    <Flex>
                      ...
                      <InputGroup w="300px">
                        <Input
                          _readOnly={{ boxShadow: '' }}
                          {...register('resourceName', { required: '请选择资源' })}
                          placeholder="请输入名称"
                        />
      ...
      {showAreaFloors && mapRef.current && position[0] && position[1] && (
        <Popup
          maxWidth="none"
          closeButton={false}
          map={mapRef.current}
