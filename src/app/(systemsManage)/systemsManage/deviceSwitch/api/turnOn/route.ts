...
export async function POST(request: Request) {
  ...
  const { id, resourceIds }: { id: string; resourceIds: Array<listType> } = await request.json();
  ...
  const url = `${process.env.NEXT_PUBLIC_ANALYTICS_Ms_Gateway}/cx-alarm/alm/turnoff/turnOnResource`;
