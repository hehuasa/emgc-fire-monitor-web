...
import Earthquake, { Refs as earthquakeType } from '@/components/Earthquake';
import Header from '@/components/Header/Header';
import { IOritreeData } from '@/components/Montior/Tree';
import WeatherWarningEffect from '@/components/WeatherWarningEffect';
...
const Layout = ({ children }: { children: ReactNode }) => {
  ...
  const getAlarmTypes = () => {
    request<{
      ...
      url: '/cx-alarm/alm/alarm_auth/now_user_data_auth',
      options: {
        method: 'post',
      },
    }).then((res) => {
  ...
  const getAlarmDealTypes = () => {
    request<IAlarmDealType[]>({
      url: '/cx-alarm/dc_dict/list_item?dictCode=deal_result_view',
    }).then((res) => {
      if (res.code === 200) {
  ...
  const getAlarmType = async () => {
    const { code, data } = await request<IAlarmTypeItem[]>({
      url: `/cx-alarm/alm/alarm/getAlarmType`,
    });
    if (code === 200) {
  ...
  // 获取区域列表
  const getAreaList = () => {
    request<IAreaTreeItem>({ url: '/ms-alarm/area/list-tree' }).then((res) => {
      if (res.code === 200) {
  ...
  const updateToken = () => {
    ...
    if (updateTokernTimer.current === undefined) {
      updateTokernTimer.current = setInterval(() => {
        request<string>({ url: '/ms-login/user/token/refresh', options: { method: 'post' } }).then(
          (res) => {
            if (res.code === 200) {
  ...
  const showLDUrl = ['/emgc/montior/operation'];
  //是否显示雷电
  const showLD = showLDUrl.find((val) => pathname?.includes(val));
    ...
    <Script strategy="afterInteractive" src="/wenet/js/SoundRecognizer.js"></Script> */}
          ...
          >
            ...
            <Box flex="1" overflow="hidden" position="relative">
              {showLD && (
                <Box position="absolute" right="90px" top="8px" zIndex={2}>
                  <WeatherWarningEffect
                    containerWidth={'72px'}
              ...
              {/* {showLD && <ThunderAndLightning position="absolute" right="34px" top="8px" zIndex={10} />} */}
              {authLoad && children}
            </Box>

            <CallNumberContainer ref={phoneSocket} />
            <Earthquake ref={earthquakeSocket} />
          </Flex>
        </Box>
      ) : (
        <NoAuth />
